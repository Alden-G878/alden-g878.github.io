...
% Margins are measured, not guessed. The master CV uses ~0.5in and runs 3
% pages; this file tightens to 0.25in, which fits the same content in 2 pages.
% Rendering it with the default 1in margins needed 5 — a real readability
% regression, since the master CV is the format recruiters already expect.
%
% So the PDF is deliberately DENSER than the master CV (2 pages vs 3). That is
% a knowing trade: the extra page came from the margin, not from dropping
% content. If a posting asks for a 3-page CV, loosen margin back toward 0.5in
% rather than re-adding anything.
%
...
% 2 pages (the master CV it mirrors runs 3). Do not tighten further without
